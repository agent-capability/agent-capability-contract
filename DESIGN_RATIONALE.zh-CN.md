...
4. 目标 Binding 的原生标准协议与 Schema 机制不能已经清晰表达它。
5. 它不要求运行时成为最终业务授权方。
...
| 跨跳主体连续性与委托 | 一个主体布尔要求无法证明身份在多个 Agent、运行时、组织、签发者和信任域之间保持不变。 | 身份、委托或协议 Binding 层 |
| 可独立验证的审批证据 | 规范化字节、签名、签发者密钥、时效和防重放需要完整的证据协议与威胁模型。 | 审批证据、证明或委托层 |
...
| 详细 effect 或业务 domain 分类 | `execution.readonly` 已表达最可移植的状态变更差异。破坏性、增量式、开放世界和业务域标签与 Binding 原生元数据存在重叠，也可能不互斥，目前尚未形成稳定的跨行业语义。 | Binding 原生标注、声明检查工具、实现扩展或未来基于证据的 Proposal |
...
### 4.1 为什么 ACC v1 使用紧凑的风险分类

`risk.level` 描述允许 Agent 自动发起某项操作时最坏的合理后果。它是可移植的分类结果，并不意味着所有风险成因都已经能够被标准化。

状态变更、破坏性、资金流转、权限修改、外部通信或业务领域等候选因素，可能有助于声明编写与诊断，但不天然适合成为核心字段：

- 同一个操作可能根据参数和当前业务状态产生多个不同效果；
- 行业领域彼此重叠，无法稳定映射为一套全球通用的风险策略；
- OpenAPI 方法、ACC execution 提示、MCP annotation 和未来 Binding 可能已经携带含义相近但信任语义不同的元数据；
- 如果新运行时依据可选因素升级 `risk.level`，而旧运行时忽略这些因素，兼容回退可能变得更不安全。

实现可以先通过带命名空间的扩展或 lint 规则验证这些因素。未来 Proposal 必须在任何因素进入 ACC Core 前，定义确定性行为、优先级、保守回退，并提供来自独立 Binding 的实现证据。

### 4.2 为什么 guidance 保留在能力契约中

面向 Agent 的能力不仅需要被治理，也必须能够被正确选择。`guidance` 提供补充性的模型可读选择上下文，同时始终保持非权威、非安全策略的定位。

当 Binding 原生的 Schema、描述和示例机制已经能够准确表达 API 时，应优先复用这些标准机制。ACC guidance 不能替代请求或响应 Schema，不能与 Binding 原生描述相矛盾，也不能成为授权策略。每个 Binding 负责定义原生描述元数据与 ACC 补充 guidance 之间的映射和优先级。
